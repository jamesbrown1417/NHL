...
# Only get NHL Games
df <- df |> filter(str_detect(competition_name, 'NHL'))

df$url <-
  paste0("https://www.neds.com.au/sports/ice-hockey/usa/nhl/",
         tolower(gsub(" ", "-", df$event_name)),
         "/",
         df$event_id)

# Write out as csv
write_csv(df, "OddsScraper/Neds/neds_nhl_match_urls.csv")
